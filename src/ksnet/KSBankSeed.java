...
class KSBankSeed
{
	...
	public KSBankSeed(String pbUserKeyStr)
	{
		byte[] buf = null;
		try{
			buf = pbUserKeyStr.getBytes("ksc5601");
		}catch(Exception e){
			e.printStackTrace();
		}
		
		//SeedEncRoundKey(sRoundKey, KSBankMsg.s2b(pbUserKeyStr));
		SeedEncRoundKey(sRoundKey, buf);
	}
...
//System.out.println("cbc_encrypt 1.패딩전 ("+sbuf.length+":"+KSBankSeed.hex_encode(sbuf)+")");

		byte[] inData = addPadding(sbuf,PKCS5_PADDING,SeedBlockSize);

//System.out.println("cbc_encrypt 2.패딩후 ("+inData.length+":"+KSBankSeed.hex_encode(inData)+")");
...
//System.out.println("cbc_encrypt 4.패딩후 ("+KSBankSeed.hex_encode(xOutput)+")");
	...
	public static String hex_encode(byte sbuf[])
	{
		...
		try{
			if(sbuf == null) 	str =  null;
			else				str =  new String(rbuf, 0, rbuf.length, "ksc5601");
